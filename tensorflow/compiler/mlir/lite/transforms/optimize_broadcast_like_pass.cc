...
bool IsScalarizableSplatConstant(Operation* op) {
  if (!llvm::isa<arith::ConstantOp, TFL::ConstOp, TFL::QConstOp>(op)) {
    return false;
  }
  ...
  auto element_type =
      mlir::dyn_cast<ShapedType>(const_value.getType()).getElementType();
  // Ignore per-axis quantized constants because after converting to scalar,
  // we will lose per-axis qantization parameter.
  if (element_type &&
      mlir::isa<quant::UniformQuantizedPerAxisType>(element_type)) {
    return false;
  }
