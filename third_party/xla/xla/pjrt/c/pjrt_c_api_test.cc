...
// offset => {"field_name", size, version}
...
FieldOffsetsAndSizesForVersion(int major_version, int minor_version) {
  ...
  if ((major_version == 0 && minor_version <= 57) ||
      (PJRT_API_MAJOR == 0 && minor_version <= PJRT_API_MINOR)) {
    ...
    add_field("extension_start", kFnPtrSize);
...
TEST_F(PjrtCAbiTestBase, FieldOffsetsAndSizes) {
  ...
  for (int minor_version = 0; minor_version <= PJRT_API_MINOR;
       ++minor_version) {
    auto offsets_and_sizes =
        FieldOffsetsAndSizesForVersion(PJRT_API_MAJOR, minor_version);
    // There should be *something* for the current API.
    if (minor_version == PJRT_API_MINOR) {
      ASSERT_FALSE(offsets_and_sizes.empty());
      auto last_field = offsets_and_sizes.back();
      size_t last_field_offset = std::get<1>(last_field);
      size_t last_field_size = std::get<2>(last_field);
      size_t api_size = last_field_offset + last_field_size;
      EXPECT_EQ(api_size, PJRT_Api_STRUCT_SIZE);
    }
    for (auto [field_name, offset, size] : offsets_and_sizes) {
      auto it = current_api_offsets_and_sizes.find(field_name);
