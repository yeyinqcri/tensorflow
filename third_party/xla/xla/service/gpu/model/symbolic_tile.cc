...
std::optional<SizeAndStrideExpression> ExtractSizeAndStrideFromMod(
  ...
  if (llvm::isa<AffineDimExpr>(lhs)) {
    // Let f(d0) = d0 mod m. Then, given an input tile size n,
    // {f(x) | x in Fin(n)} contains min(n, m) elements,
    // with stride 1, and constraints that one of the following holds:
    // - m is a multiple of n.
    // - n is a multiple of m.
    //
    // The right hand side of the minimum is expressed as:
    // m = n - n + m = n - ((n - 1) floordiv m) * m,
    // which holds because n % m == 0.
    AffineExpr size = lhs - (lhs - 1).floorDiv(modulus) * modulus;
    // m % n == 0 || n % m == 0.
    ConstraintExpression constraints =
        Constraint{modulus % lhs, zero_interval} ||
        Constraint{lhs % modulus, zero_interval};
  ...
  // {f(x) | x in Fin(n)} contains min(n ceildiv d, m) elements,
  // with stride 1, and constraints that one of the following holds:
  // - n is a multiple of d and d*m is a multiple of n.
  // - n is a multiple of d*m.
  // - d is a multiple of n.
  //
  // The right hand side of the minimum is expressed as:
  // m = n ceildiv d - n ceildiv d + m
  //   = n ceildiv d - ((n - 1) floordiv (d * m)) * m
  // which holds because n % (d * m) == 0.
  //
  ...
  // (n % d == 0 && d*m % n == 0) || d % n == 0 || n % (d*m) == 0.
  ConstraintExpression constraints =
      Constraint{dim % den, zero_interval} &&
      Constraint{den * modulus % dim, zero_interval};
  constraints = constraints || Constraint{dim % (den * modulus), zero_interval};
  constraints = constraints || Constraint{den % dim, zero_interval};
