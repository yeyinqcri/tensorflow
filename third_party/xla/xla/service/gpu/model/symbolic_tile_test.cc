...
TEST_F(SymbolicTileTest, CanPropagateTileModAndFloorDiv) {
  ...
  EXPECT_THAT(SymbolicTile::FromIndexingMap(indexing_map),
      ...
      Symbolic tile with
        ...
        stride_map: (d0) -> (1, 1, 1)
        constraints: 35 mod d0 in [0, 0] && 7 mod d0 in [0, 0] && d0 mod 7 in [0, 0] || 35 mod d0 in [0, 0] && d0 mod 7 in [0, 0] || 7 mod d0 in [0, 0] || 7 mod d0 in [0, 0] && d0 mod 35 in [0, 0] || 7 mod d0 in [0, 0] && d0 mod 7 in [0, 0] || d0 mod 35 in [0, 0] && d0 mod 7 in [0, 0]
      )")));
}
