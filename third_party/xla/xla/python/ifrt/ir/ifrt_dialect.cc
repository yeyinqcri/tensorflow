...
mlir::LogicalResult IfrtArrayType::verify(
    ...
    mlir::StringAttr layout_mode_attr) {
  if (layout_mode_attr) {
    auto layout_mode = xla::LayoutMode::FromString(layout_mode_attr.str());
    if (!layout_mode.ok()) {
...
xla::LayoutMode IfrtArrayType::LayoutMode() const {
  if (auto layout_mode_attr = getLayoutModeAttr()) {
    auto layout_mode = xla::LayoutMode::FromString(layout_mode_attr.str());
    CHECK_OK(layout_mode) << "Invalid layout mode: " << layout_mode_attr.str();
    return *layout_mode;
  }
  // Use the default layout mode if it wasn't specified.
  return xla::LayoutMode::FromString("default").value();
}
...
void IfrtArrayType::print(mlir::AsmPrinter& odsPrinter) const {
  ...
  if (getMemoryKindAttr() != mlir::StringAttr()) {
    odsPrinter << ", memory_kind = ";
    odsPrinter.printStrippedAttrOrType(getMemoryKindAttr());
  }
  if (getLayoutModeAttr() != mlir::StringAttr()) {
    odsPrinter << ", layout_mode = ";
    odsPrinter.printStrippedAttrOrType(getLayoutModeAttr());
  }
  odsPrinter << ">";
}
...
mlir::FailureOr<mlir::StringAttr> parseLayoutModeAttr(
    mlir::AsmParser& odsParser) {
  if (mlir::failed(odsParser.parseOptionalKeyword("layout_mode")))
    return mlir::failure();
  if (mlir::failed(odsParser.parseEqual())) return mlir::failure();
  auto layout_mode_attr_or =
      mlir::FieldParser<mlir::StringAttr>::parse(odsParser);
  if (mlir::failed(layout_mode_attr_or)) {
    odsParser.emitError(
        odsParser.getCurrentLocation(),
        "failed to parse Ifrt_ArrayType parameter 'layout_mode_attr' which "
        "is to be a `mlir::StringAttr`");
    return mlir::failure();
  }
  return layout_mode_attr_or;
}
...
mlir::Type IfrtArrayType::parse(mlir::AsmParser& odsParser) {
  ...
  mlir::FailureOr<mlir::StringAttr> layout_mode_attr_or;
  if (mlir::succeeded(odsParser.parseOptionalComma())) {
    memory_kind_attr_or = parseMemoryKindAttr(odsParser);
    if (mlir::failed(memory_kind_attr_or)) {
      layout_mode_attr_or = parseLayoutModeAttr(odsParser);
      if (mlir::failed(layout_mode_attr_or)) {
        odsParser.emitError(
            odsParser.getCurrentLocation(),
    ...
    if (mlir::succeeded(odsParser.parseOptionalComma())) {
      ...
      if (mlir::failed(layout_mode_attr_or)) {
        odsParser.emitError(
            odsParser.getCurrentLocation(),
            "failed to parse Ifrt_ArrayType `layout_mode` attributes");
        return {};
      }
    }
  }
  ...
  return odsParser.getChecked<IfrtArrayType>(
      ...
      *sharding_attr_or, *devices_attr_or,
      memory_kind_attr_or.value_or(mlir::StringAttr()),
      layout_mode_attr_or.value_or(mlir::StringAttr()));
}
